...
func _draw() -> void:
	...
	for node_variant: Variant in nodes:
		...
		var travel := 0.5 + 0.5 * sin(_pulse * 1.7 + target.x * 0.01)
...
func _draw_node(position: Vector2, title: String, value: String, accent: Color, radius: float) -> void:
	var pulse_radius := radius + 4.0 + 2.5 * sin(_pulse * 2.0 + position.x * 0.02)
	...
	draw_string(font, position + Vector2(-width * 0.5, -4.0), title, HORIZONTAL_ALIGNMENT_CENTER, width, 10, accent)
	draw_string(font, position + Vector2(-width * 0.5, 13.0), _shorten(value, 18), HORIZONTAL_ALIGNMENT_CENTER, width, 9, COLOR_TEXT)
