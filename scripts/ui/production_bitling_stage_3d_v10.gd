...
func _build_living_home_layer() -> void:
	...
	var window_material: StandardMaterial3D = _emissive_material(Color("071631"), Color("26d9ff"), 1.8)
	...
	for index in range(6):
		var tower: MeshInstance3D = _mesh(_box(Vector3(0.45 + float(index % 2) * 0.18, 1.2 + float(index % 3) * 0.45, 0.16)), _emissive_material(Color("081226"), Color("7f4dff"), 1.5), Vector3(-2.1 + float(index) * 0.82, 0.55 + float(index % 3) * 0.18, -2.92), _home_root)
		tower.name = "WindowTower%02d" % index

	var lamp_mesh: MeshInstance3D = _mesh(_sphere(0.24, 18, 12), _emissive_material(Color("17102f"), Color("a855f7"), 4.0), Vector3(-2.45, 2.35, -0.55), _home_root)
	...
	for index in range(7):
		...
		var leaf: MeshInstance3D = _mesh(_capsule(0.08, 0.58, 12, 6), _emissive_material(Color("06221c"), Color("55f0a8"), 1.6), Vector3(cos(angle) * 0.22, 0.48 + float(index % 2) * 0.12, sin(angle) * 0.16), _plant_root)
		leaf.rotation_degrees.z = rad_to_deg(angle) + 18.0

	for index in range(6):
		var orb: MeshInstance3D = _mesh(_sphere(0.08, 12, 8), _emissive_material(Color("140b2e"), Color("ff5ecb"), 2.4), Vector3(-1.65 + float(index) * 0.34, 1.05 + float(index % 2) * 0.20, -1.85), _home_root)
		orb.visible = index == 0
		_memory_orbs.append(orb)
	for index in range(18):
		var mote: MeshInstance3D = _mesh(_sphere(0.018 + float(index % 3) * 0.006, 8, 5), _emissive_material(Color("071426"), Color("42e8ff"), 1.3), Vector3(-2.6 + float(index % 6) * 1.0, 0.45 + float(index / 6) * 0.72, -1.2 + sin(float(index)) * 0.6), _home_root)
		_dust_motes.append(mote)

func _connect_living_home() -> void:
	var service: Node = get_node_or_null("/root/LivingHome")
...
func _apply_home_palette() -> void:
	if _lamp_orb == null:
		return
	var mode: String = str(_home_snapshot.get("light_mode", "CYAN"))
	var color_variant: Variant = {"CYAN": Color("42e8ff"), "VIOLET": Color("a855f7"), "SUNSET": Color("ff8b5c"), "MOON": Color("8da8ff")}.get(mode, Color("42e8ff"))
	var color: Color = color_variant as Color
	_lamp_orb.light_color = color
	if _window_panel != null:
		var material := _window_panel.material_override as StandardMaterial3D
		if material != null:
			var weather: String = str(_home_snapshot.get("weather", "CLEAR"))
			var emission_variant: Variant = {"CLEAR": Color("26d9ff"), "RAIN": Color("4f7cff"), "AURORA": Color("9b5cff"), "STORM": Color("ff4fc8")}.get(weather, Color("26d9ff"))
			material.emission = emission_variant as Color

func _apply_home_objects() -> void:
	if _plant_root != null:
		var health: float = clampf(float(_home_snapshot.get("plant_health", 70.0)) / 100.0, 0.25, 1.0)
		_plant_root.scale = Vector3(0.88 + health * 0.18, 0.72 + health * 0.36, 0.88 + health * 0.18)
	var displayed_variant: Variant = _home_snapshot.get("displayed_memories", [])
	var displayed: Array = displayed_variant as Array
	var visible_memories: int = mini(displayed.size() + 1, _memory_orbs.size())
...
func _animate_living_home(delta: float) -> void:
	...
	for index in range(_memory_orbs.size()):
		var orb: Node3D = _memory_orbs[index]
		orb.position.y += sin(_home_clock * 1.1 + float(index)) * delta * 0.018
		orb.rotation.y += delta * (0.3 + float(index) * 0.04)
	var cleanliness: float = clampf(float(_home_snapshot.get("cleanliness", 75.0)) / 100.0, 0.0, 1.0)
	for index in range(_dust_motes.size()):
		var mote: Node3D = _dust_motes[index]
