...
const INTENT_KEYS := {
	"greet": "intent.greet",
	"invite_play": "intent.invite_play",
	"share_discovery": "intent.share_discovery",
	"comfort": "intent.comfort",
	"celebrate": "intent.celebrate",
	"ask_question": "intent.ask_question",
	"explain_pattern": "intent.explain_pattern",
	"tell_joke": "intent.tell_joke",
	"debate": "intent.debate",
	"monologue": "intent.monologue",
	"teach": "intent.teach",
...
func normalize_locale(locale: String) -> String:
	var cleaned := locale.strip_edges().replace("_", "-").to_lower()
	if cleaned.is_empty():
...
func translate(key: String, locale: String = "", values: Dictionary = {}) -> String:
	var requested := normalize_locale(locale if not locale.is_empty() else current_locale)
	var text := _lookup(requested, key)
...
func translate_intent(intent: String, locale: String = "", payload: Dictionary = {}) -> String:
	var key := str(INTENT_KEYS.get(intent, "intent.unknown"))
	return translate(key, locale, payload)

func render_bitling_speech(intent: String, payload: Dictionary = {}, locale: String = "") -> Dictionary:
	var target_locale := normalize_locale(locale if not locale.is_empty() else current_locale)
	var emotion_name := str(payload.get("emotion", "calm"))
	var utterance := "Bii-luma!"
	var language := get_node_or_null("/root/BitlingLanguage")
	if language != null and language.has_method("create_packet"):
		var identity_id := "bitling"
	...
	var subtitle := translate_intent(intent, target_locale, payload)
	var profile := get_node_or_null("/root/DevelopmentProfile")
	var human_speech := ""
	var can_speak_human := profile != null and profile.has_legendary_language()
	if can_speak_human:
		human_speech = subtitle
	return {
		"intent": intent,
		"bitling_utterance": utterance,
		"subtitle": subtitle,
...
func decode_peer_packet(packet: Dictionary, locale: String = "") -> Dictionary:
	var intent := str(packet.get("intent", "unknown"))
...
func create_bitling_language_lesson(difficulty: int = 1, locale: String = "") -> Dictionary:
	...
	var index := clampi(difficulty - 1, 0, lessons.size() - 1)
...
func get_language_capabilities() -> Dictionary:
	var profile := get_node_or_null("/root/DevelopmentProfile")
	var legendary := profile != null and profile.has_legendary_language()
	return {
		"semantic_understanding": true,
		"requested_locale": current_locale,
		"fallback_locale": FALLBACK_LOCALE,
		"available_translation_locales": translations.keys(),
		"bitling_language_teaching": true,
		"human_speech": legendary,
...
func _load_builtin_translations() -> void:
	translations = {
		...
		"en": {
			...
			"lesson.bii": "friendly attention",
			"lesson.luma": "light, joy, or a warm greeting",
			"lesson.zumi": "excitement and curiosity",
			"lesson.nobi": "a question or request",
			"lesson.plonk": "a clumsy surprise"
		},
		"es": {
			"intent.greet": "¡Hola! Me alegra verte.",
			"intent.invite_play": "¿Jugamos juntos?",
			"intent.goodbye": "Hasta pronto. Recordaré nuestra conversación."
		},
		"fr": {
			"intent.greet": "Bonjour ! Je suis heureux de te voir.",
			"intent.invite_play": "Veux-tu jouer avec moi ?",
			"intent.goodbye": "À bientôt. Je me souviendrai de notre conversation."
		},
		"it": {
			"intent.greet": "Ciao! Sono felice di vederti.",
			"intent.invite_play": "Giochiamo insieme?",
			"intent.goodbye": "A presto. Ricorderò la nostra conversazione."
		},
		"pt": {
			"intent.greet": "Olá! Fico feliz em ver você.",
			"intent.invite_play": "Vamos brincar juntos?",
			"intent.goodbye": "Até logo. Vou lembrar da nossa conversa."
		},
		"pl": {
			"intent.greet": "Cześć! Miło cię widzieć.",
			"intent.invite_play": "Pobawimy się razem?",
			"intent.goodbye": "Do zobaczenia. Zapamiętam naszą rozmowę."
		},
		"tr": {
			"intent.greet": "Merhaba! Seni gördüğüme sevindim.",
			"intent.invite_play": "Birlikte oynayalım mı?",
			"intent.goodbye": "Yakında görüşürüz. Konuşmamızı hatırlayacağım."
		},
		"ru": {
			"intent.greet": "Привет! Я рад тебя видеть.",
			"intent.invite_play": "Давай поиграем вместе?",
			"intent.goodbye": "До скорого. Я запомню наш разговор."
		},
		"ja": {
			"intent.greet": "こんにちは。会えてうれしいです。",
			"intent.invite_play": "いっしょに遊びませんか？",
			"intent.goodbye": "またね。この会話を覚えておくよ。"
		},
		"ko": {
			"intent.greet": "안녕! 만나서 반가워.",
			"intent.invite_play": "같이 놀래?",
			"intent.goodbye": "다음에 봐. 우리 대화를 기억할게."
		},
		"zh": {
			"intent.greet": "你好！很高兴见到你。",
			"intent.invite_play": "我们一起玩好吗？",
			"intent.goodbye": "再见。我会记住我们的谈话。"
		},
		"ar": {
			"intent.greet": "مرحبًا! سعيد برؤيتك.",
			"intent.invite_play": "هل نلعب معًا؟",
			"intent.goodbye": "إلى اللقاء. سأتذكر حديثنا."
		},
		"hi": {
			"intent.greet": "नमस्ते! तुम्हें देखकर खुशी हुई।",
			"intent.invite_play": "क्या हम साथ खेलें?",
			"intent.goodbye": "फिर मिलेंगे। मैं हमारी बातचीत याद रखूँगा।"
		}
	}
