...
## Individual development model for one Bitling.
## The intelligence quotient, attributes, skills and preferences belong to the
## Bitling itself and are never derived from the human player's performance.
...
func reset_state() -> void:
	var identity_id := _identity_id()
	var seed_value := abs(hash(identity_id if not identity_id.is_empty() else "new-bitling"))
	var rng := RandomNumberGenerator.new()
	rng.seed = seed_value
	intelligence_quotient = rng.randi_range(82, 128)
	iq_growth_points = 0.0
	attributes = {
		"intelligence": float(intelligence_quotient),
		"empathy": rng.randf_range(42.0, 68.0),
		"humor": rng.randf_range(38.0, 72.0),
		"coordination": rng.randf_range(34.0, 66.0),
		"discipline": rng.randf_range(24.0, 48.0),
		"creativity": rng.randf_range(45.0, 76.0),
		"charisma": rng.randf_range(36.0, 70.0),
		"resilience": rng.randf_range(40.0, 70.0),
...
func record_interaction(interaction_id: String, tags: Array[String] = [], quality: float = 1.0) -> void:
	var amount := clampf(quality, 0.1, 5.0)
	var normalized := interaction_id.to_lower()
...
func progress_specialization(specialization_id: String, amount: float) -> Dictionary:
	...
	var old_rank := int(entry.get("rank", RANK_BRONZE))
...
func get_autonomy_score() -> float:
	return clampf(
		...
		+ float(skills.get("self_care", {}).get("rating", 0.0)) * 0.10,
		0.0,
		100.0
	)
...
func can_teach_peer() -> bool:
	return get_autonomy_score() >= 58.0 and float(skills.get("teaching", {}).get("rating", 0.0)) >= 38.0
...
func choose_autonomous_action(peer_available: bool = false) -> Dictionary:
	...
	var autonomy := get_autonomy_score()
	...
	var seed_value := hash("%s:%d:%d" % [_identity_id(), int(Time.get_unix_time_from_system() / 300.0), social_history.size()])
	var rng := RandomNumberGenerator.new()
	rng.seed = seed_value
	var total := 0.0
...
func calculate_affinity(peer_profile: Dictionary) -> Dictionary:
	...
	var hobby_similarity := _array_similarity(preferences.get("hobbies", []), peer_preferences.get("hobbies", []))
	var food_match := 1.0 if str(preferences.get("favorite_food", "")) == str(peer_preferences.get("favorite_food", "")) else 0.0
	var style_match := 1.0 if str(preferences.get("conversation_style", "")) == str(peer_preferences.get("conversation_style", "")) else 0.0
	var attribute_similarity := _dictionary_similarity(attributes, peer_attributes)
	var skill_similarity := _skill_similarity(skills, peer_skills)
	var score := clampf(
		hobby_similarity * 30.0
		+ food_match * 12.0
		+ style_match * 8.0
		+ attribute_similarity * 30.0
...
func register_social_encounter(peer_id: String, peer_profile: Dictionary) -> Dictionary:
	...
	var affinity := calculate_affinity(peer_profile)
...
func choose_conversation_mode(peer_profile: Dictionary = {}) -> String:
	var affinity := float(calculate_affinity(peer_profile).get("score", 50.0)) if not peer_profile.is_empty() else 50.0
	var debate_rating := float(skills.get("debate", {}).get("rating", 0.0))
	var teaching_rating := float(skills.get("teaching", {}).get("rating", 0.0))
	var humor_rating := float(skills.get("humor", {}).get("rating", 0.0))
	if can_teach_peer() and teaching_rating >= 55.0:
		return "teach"
	if debate_rating >= 50.0 and affinity >= 45.0:
		return "debate"
	if humor_rating >= 48.0:
		return "joke"
	if affinity >= 70.0:
		return "discussion"
...
func get_age_adjusted_style() -> Dictionary:
	match player_age_band:
		"child":
			return {"vocabulary": "simple", "complexity": 0.35, "max_monologue_seconds": 25, "sensitive_topics": false}
		"teen":
			return {"vocabulary": "casual", "complexity": 0.65, "max_monologue_seconds": 45, "sensitive_topics": true}
		"senior":
			return {"vocabulary": "clear", "complexity": 0.72, "max_monologue_seconds": 55, "sensitive_topics": true}
		_:
			return {"vocabulary": "standard", "complexity": 0.82, "max_monologue_seconds": 60, "sensitive_topics": true}
...
func get_display_snapshot() -> Dictionary:
	return {
		...
		"specializations": specializations.duplicate(true),
		"upbringing": upbringing.duplicate(true),
		"autonomy_score": get_autonomy_score(),
		"autonomy_efficiency": get_autonomous_action_efficiency(),
		"preferences": preferences.duplicate(true),
		"favorite_bitling_id": favorite_bitling_id,
		"favorite_bitling_affinity": favorite_bitling_affinity,
		"rarity": rarity.duplicate(true),
		"player_age_band": player_age_band,
		"age_style": get_age_adjusted_style()
	}
...
func export_state() -> Dictionary:
	return {
		...
		"intelligence_quotient": intelligence_quotient,
		"iq_growth_points": iq_growth_points,
		"attributes": attributes.duplicate(true),
		"skills": skills.duplicate(true),
		"abilities": abilities.duplicate(true),
		"specializations": specializations.duplicate(true),
		...
		"rarity": rarity.duplicate(true),
		"player_age_band": player_age_band,
		"social_history": social_history.duplicate(true)
	}
...
func _update_abilities() -> void:
	...
	_unlock_if("structured_debate", float(skills.get("debate", {}).get("rating", 0.0)) >= 50.0)
	_unlock_if("long_monologue", float(skills.get("language", {}).get("rating", 0.0)) >= 55.0)
	_unlock_if("bitling_language_teacher", float(skills.get("teaching", {}).get("rating", 0.0)) >= 65.0)
...
func _create_rarity(seed_value: int) -> Dictionary:
	var roll := abs(seed_value) % 10000
	var tier := "COMMON"
	var multiplier := 1.0
	var visual := {"shimmer": 0.0, "glow": 0.0, "sparkles": false, "hue_shift": 0.0}
	if roll < 25:
		tier = "LEGENDARY"
		multiplier = 1.12
...
func _pick_unique(pool: Array, rng: RandomNumberGenerator, count: int) -> Array[String]:
	...
	while not copy.is_empty() and result.size() < count:
		var index := rng.randi_range(0, copy.size() - 1)
...
func _skill_similarity(a: Dictionary, b: Dictionary) -> float:
	...
	for key in a.keys():
		if b.has(key):
			var ar := float(a[key].get("rating", 0.0))
			var br := float(b[key].get("rating", 0.0))
			total += 1.0 - abs(ar - br) / 100.0
			count += 1
	return total / float(maxi(count, 1))

func _affinity_label(score: float) -> String:
	if score >= 85.0:
		return "SEELENFREQUENZ"
	if score >= 70.0:
