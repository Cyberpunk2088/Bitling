...
func _test_streak_service() -> void:
	var streak = root.get_node("StreakService")
	streak.import_state({})
	var started: Dictionary = streak.register_activity(_date(2026, 1, 1))
	_assert(started.status == "started", "Streak starts on first activity")
	...
	_assert(continued.status == "continued", "Consecutive day continues streak")
	_assert(streak.current_streak == 2, "Consecutive streak increments")

	var gap: Dictionary = streak.register_activity(_date(2026, 1, 4))
	_assert(gap.status == "recovery_available", "Missed day offers non-punitive recovery")
...
func _test_quest_service() -> void:
	...
	var reward := quests.claim_reward(str(first[0].get("id", "")))
	_assert(reward > 0, "Completed quest reward can be claimed once")
	_assert(quests.claim_reward(str(first[0].get("id", ""))) == 0, "Quest reward cannot be claimed twice")

func _test_companion_brain() -> void:
	var brain = root.get_node("CompanionBrain")
	brain.reset_state()
	var old_relationship: float = brain.relationship_score
	var old_empathy: float = float(brain.personality.empathy)
	brain.observe_interaction("care", 1.0, {"source": "ci"})
	_assert(brain.relationship_score > old_relationship, "Care increases relationship")
	_assert(float(brain.personality.empathy) > old_empathy, "Care shapes companion personality")
	_assert(brain.interaction_counts.get("care", 0) == 1, "Interaction frequency is remembered")

	var exported: Dictionary = brain.export_state()
	brain.reset_state()
	brain.import_state(exported)
	_assert(brain.interaction_counts.get("care", 0) == 1, "Companion memory survives export/import")
	_assert(not brain.get_greeting().is_empty(), "Companion always produces a greeting")

func _test_wellbeing_guard() -> void:
	var guard = root.get_node("WellbeingGuard")
...
func _test_game_state_progression_and_interactions() -> void:
	...
	_assert(state.level == 3, "Large XP grants multiple level-ups")
	_assert(state.xp == 50, "XP remainder is preserved")

	var old_hunger: float = state.hunger
	var old_happiness: float = state.happiness
	...
	_assert(state.hunger > old_hunger, "Care interaction updates saturation")
	_assert(state.happiness > old_happiness, "Care interaction updates happiness")
	_assert(state.total_xp == 265, "Interaction XP is included in total XP")
	_assert(root.get_node("CompanionBrain").interaction_counts.get("care", 0) >= 1, "Game interaction reaches companion brain")
...
func _test_save_roundtrip() -> void:
	...
	var saved: bool = state.save_game_state()
	_assert(saved, "Game state saves successfully")
	state.level = 1
	state.xp = 0
	state.story_flags.erase("ci_roundtrip")
	var loaded: bool = state.load_game_state()
	_assert(loaded, "Game state loads successfully")
	_assert(state.level == 7 and state.xp == 42, "Progression survives save roundtrip")
